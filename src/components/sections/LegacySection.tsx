...
export default function LegacySection() {
  ...
  return (
    <section
      ref={sectionRef}
      className="relative py-28 md:py-40 bg-cream overflow-hidden"
    >
      {/* Ambient background glow */}
      <div className="absolute inset-0 pointer-events-none">
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[700px] h-[500px] bg-gold/[0.08] blur-[160px] rounded-full" />
      </div>

      {/* Subtle side accents */}
      <div className="absolute top-0 left-0 w-px h-full bg-gradient-to-b from-transparent via-gold/15 to-transparent pointer-events-none" />
      <div className="absolute top-0 right-0 w-px h-full bg-gradient-to-b from-transparent via-gold/15 to-transparent pointer-events-none" />

      <div className="max-w-4xl mx-auto px-4 md:px-8 text-center relative">
        {/* Top decorative line */}
        <div className="legacy-line-top w-24 h-px bg-gold/40 mx-auto mb-12 origin-center" />

        {/* Diamond decorations with logo icon */}
        <div className="flex items-center justify-center gap-4 mb-10">
          <div className="legacy-diamond w-1.5 h-1.5 bg-gold/50 rotate-45 opacity-0" />
          <div className="legacy-diamond w-2 h-2 bg-gold/50 rotate-45 opacity-0" />
          <svg
        ...
        {/* Section label */}
        <span
          className="legacy-label text-gold-dark/70 text-xs tracking-[0.3em] uppercase block mb-6 opacity-0"
        ...
        >
          ...
          <div className="w-10 h-px bg-gold/50 group-hover:w-16 transition-all duration-500" />
          <ArrowRight
            size={16}
            className="text-gold/70 group-hover:translate-x-1 group-hover:text-gold transition-all duration-300"
          />
        </a>

        {/* Bottom decorative line */}
        <div className="legacy-line-bottom w-24 h-px bg-gold/40 mx-auto mt-14 origin-center" />
      </div>
    </section>
  );
}
